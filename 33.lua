...
local images = {
    ...
    -- https://raw.githubusercontent.com/yukvx/robloxlol/refs/heads/main/gamesneeze/espPreview.lua
            ...
            --
            function window:Notification(info, lifetime)
                window.notification = {visible = false}

                local notification_name = info.name or info.Name or info.title or info.Title or "Notification"
                
                local display_text = notification_name
                if lifetime and type(lifetime) == "number" then
                    display_text = string.format("%s (%ds)", notification_name, lifetime)
                end
            
                local text_bounds = utility:GetTextBounds(display_text, theme.textsize, theme.font)
                ...
                function window.notification:Visibility(visible)
                    notification_outline.Visible = visible
                    notification_inline.Visible = visible
                    notification_frame.Visible = visible
                    notification_accent.Visible = visible
                    notification_title.Visible = visible
                end
            
                window.notification:UpdateSize()
                window.notification:Visibility(true)

                if lifetime and type(lifetime) == "number" then
                    task.delay(lifetime, function()
                        window.notification:Visibility(false)
                    end)
                end
