...
do 
    ...
    function notifications.UpdateNotifications()
        ...
        for _, notification in pairs(notifications) do
            if notification and notification.holder then
                local newPosition = Vector2.new(20, 75 + (i * 30))
                local tweenInfo = TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                local lTween = Tween.new(notification.holder, tweenInfo, {Position = newPosition})
                lTween:Play()
                i = i + 1
            end
        end
    ...
    function notifications.UpdateNotifications2(drawing)
        if not drawing or type(drawing) ~= "table" then
            warn("Invalid 'drawing' parameter passed to UpdateNotifications2")
            return
        end
        ...
        for _, v in pairs(drawing) do
            if type(v) == "table" and v.Transparency ~= nil then
                ...
                local llTween = Tween.new(v, tweenInfo, {Transparency = 0})
                llTween:Play()
            end
        end
    end
