...
function window:Notification(info)
    window.notification = {visible = false}
    --
    local info = info or {}
    local notification_title = info.title or "Notification"
    local notification_message = info.message or "Message here"
    local notification_duration = info.duration or 5  -- Duration in seconds
    
    local text_bounds_title = utility:GetTextBounds(notification_title, theme.textsize, theme.font)
    local text_bounds_message = utility:GetTextBounds(notification_message, theme.textsize, theme.font)
    local max_width = math.max(text_bounds_title.X, text_bounds_message.X)

    --
    local notification_outline = utility:Create("Frame", {Vector2.new(100, 38 / 2 - 10)}, {
        Size = utility:Size(0, max_width + 20, 0, 60),  -- Adjust height for title and message
        Position = utility:Position(0, 100, 0, 38 / 2 - 10),
        Hidden = true,
        ZIndex = 60,
        Color = theme.outline,
        Visible = window.notification.visible
    })
    window.notification.outline = notification_outline
    --
    library.colors[notification_outline] = {
        Color = "outline"
    }
    --
    ...
    --
    library.colors[notification_inline] = {
        Color = "inline"
    }
    --
    ...
    --
    library.colors[notification_accent] = {
        Color = "accent"
    }
    --
    local notification_title_label = utility:Create("TextLabel", {Vector2.new(6, 6), notification_frame}, {
        ...
        Hidden = true,
        ZIndex = 61,
        Position = utility:Position(0, 6, 0, 6, notification_frame),
        Visible = window.notification.visible
    })
    --
    local notification_message_label = utility:Create("TextLabel", {Vector2.new(6, 30), notification_frame}, {
        ...
        Hidden = true,
        ZIndex = 61,
        Position = utility:Position(0, 6, 0, 30, notification_frame),
        Visible = window.notification.visible,
        TextWrapped = true  -- Wrap text for long messages
    })
    --
    library.colors[notification_title_label] = {
        OutlineColor = "textborder",
        Color = "textcolor"
    }
    library.colors[notification_message_label] = {
        OutlineColor = "textborder",
        Color = "textcolor"
    }
    --
    function window.notification:UpdateSize()
        local height = 60  -- Base height
        notification_outline.Size = utility:Size(0, max_width + 20, 0, height)
        ...
        notification_accent.Size = utility:Size(1, 0, 0, 1, notification_frame)
    end
    --
    function window.notification:Visibility()
        notification_outline.Visible = window.notification.visible
        notification_inline.Visible = window.notification.visible
        notification_frame.Visible = window.notification.visible
        notification_accent.Visible = window.notification.visible
        notification_title_label.Visible = window.notification.visible
        notification_message_label.Visible = window.notification.visible
    end
    --
    function window.notification:Update(updateType, updateValue)
        if updateType == "Visible" then
            window.notification.visible = updateValue
            window.notification:Visibility()
        end
    end
    --
    window.notification:UpdateSize()
    --
    task.spawn(function()
