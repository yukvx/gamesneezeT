...
loadstring(game:HttpGet("https://raw.githubusercontent.com/yukvx/cat.girl/refs/heads/main/library%20assets/tDrawing.lua"))()
--
local libraryAssets = {
Gradient = {
    ...
    LPH_NO_VIRTUALIZE(function()
        ...
        do
            ...
            function utility:LoadImage(instance, imageName, imageLink)
                local data
                --
                if isfile(library.folders.assets.."/"..imageName..".clrd") then
                    data = readfile(library.folders.assets.."/"..imageName..".clrd")
                else
                    if imageLink then
                        data = game:HttpGet(imageLink)
                        writefile(library.folders.assets.."/"..imageName..".clrd", data)
                    else
                        return
                    end
                end
                ...
                library.began[#library.began + 1] = function(Input)
                    if Input.KeyCode == Enum.KeyCode.P then
                        ...
                        if #plrs:GetPlayers() <= 1 then
                            plr:Kick("\nRejoining...")
                            wait()
                            clonerefgame:GetService('TeleportService')):Teleport(game.PlaceId, plr)
                        else
                            cloneref(game:GetService('TeleportService')):TeleportToPlaceInstance(game.PlaceId, game.JobId, plr)
                        end
    ...
    local notifications = {}; do 
        function notifications.UpdateNotifications()
            ...
            for v in next, notifications do
                if v.holder then 
                    local lTween = Tween.new(v.holder, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = Vector2.new(20, 75 + (i * 30))})
                    lTween:Play()
                    i = i + 1
                end
            end
        ...
        function notifications.UpdateNotifications2(drawing)
            for i,v in pairs(drawing) do
                if type(v) == "table" then
                    local llTween = Tween.new(v, TweenInfo.new(1, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Transparency = 0})
                    llTween:Play()
                end
            end
        end
